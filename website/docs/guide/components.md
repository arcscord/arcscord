...
`loadComponents` is async and returns a [`Result`](./error-handling.md) with the number of loaded handlers. An invalid component definition or a duplicate/invalid route surfaces as an `ArcscordError` failure (`COMPONENT_VALIDATION_FAILED`, `COMPONENT_ROUTE_DUPLICATE`, or `COMPONENT_ROUTE_INVALID`) instead of throwing:
